  ...
  return (
    <article
      className={`
        ...
        ${
          isUnlocked
            ? "bg-white/50 border-white/70 backdrop-blur-3xl shadow-sm"
    ...
    <main className="min-h-screen w-full bg-[#EAF6F3] pt-20">
      <div className="mx-auto w-full max-w-5xl">
        ...
        <motion.button
          ...
          onClick={() => navigate(-1)}
        > */}
        {/* <ArrowLeft size={24} className="text-[#2D3142]" /> */}
        {/* </motion.button> */}
        ...
        <div className="mx-auto max-w-[450px] p-5 lg:max-w-6xl lg:px-8 lg:py-8">
          {/* Filter Chips */}
          <div className="mb-8 flex justify-center gap-2 overflow-x-auto pb-3 no-scrollbar lg:mb-10">
            {filterChips.map((chip) => (
              <button
                key={chip.id}
                onClick={() => setActiveFilter(chip.id)}
                className={`
                hover:bg-[#ffffff] hover:text-[#334155] hover:border-white cursor-pointer whitespace-nowrap rounded-full px-5 py-1.5 text-[13px] font-medium transition-all border lg:px-6 lg:py-2 lg:text-sm
                ${
                  activeFilter === chip.id
                    ? "bg-[#7CA9A0] text-white border-[#7CA9A0] shadow-md shadow-teal-900/10"
                    : "bg-white/70 text-slate-500 border-white/90"
                }
              `}
              >
                {chip.label}
          ...
          >
            ...
            >
              ...
              <button
                type="button"
                onClick={() => setSelectedBadge(null)}
                className="cursor-pointer mt-8 w-full rounded-2xl bg-[#84B0A7] py-3.5 text-[16px] font-bold text-white shadow-lg shadow-teal-900/10 active:scale-[0.98] transition-transform"
              >
                Close
              </button>
            </div>
