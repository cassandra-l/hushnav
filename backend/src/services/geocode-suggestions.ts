...
function normaliseGisgraphyFeature(
  item: any,
  index: number,
): LocationSuggestion | null {
...
async function fetchPhotonSuggestions(
  ...
  return features
    .map((feature: any, index: number) =>
      normalisePhotonFeature(feature, index),
    )
    .filter(
...
async function fetchGisgraphySuggestions(
  query: string,
): Promise<LocationSuggestion[]> {
  const params = new URLSearchParams({
    q: query,
  ...
  return records
    .map((item: any, index: number) =>
      normaliseGisgraphyFeature(item, index),
    )
    .filter(
      (item: LocationSuggestion | null): item is LocationSuggestion =>
        item !== null,
...
export async function geocodeSuggestions(
  ...
  if (!trimmed || trimmed.length < 2 || trimmed === "Current Location") {
    return [];
  }

  let provider = "photon";
  let suggestions = await fetchPhotonSuggestions(trimmed);

  if (suggestions.length === 0) {
    provider = "gisgraphy";
    suggestions = await fetchGisgraphySuggestions(trimmed);
  }

  console.log(
    `[geocode-suggestions] query="${trimmed}", provider=${provider}, count=${suggestions.length}`,
  );
